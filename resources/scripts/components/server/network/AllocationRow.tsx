...
const AllocationRow = ({ allocation }: Props) => {
    ...
    const onNotesChanged = useCallback((id: number, notes: string) => {
        mutate(data => data?.map(a => a.id === id ? { ...a, notes } : a), false).then();
    }, []);
    ...
    const setPrimaryAllocation = () => {
        clearFlashes('server:network');
        mutate(data => data?.map(a => ({ ...a, isDefault: a.id === allocation.id })), false).then();
            ...
            <div css={tw`flex items-center w-full md:w-auto`}>
                ...
                <div css={tw`mr-4 flex-1 md:w-40`}>
                    {allocation.alias ?
                        ...
                        <CopyOnClick text={allocation.ip}><Code>{formatIp(allocation.ip)}</Code></CopyOnClick>}
